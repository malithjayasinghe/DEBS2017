...
public class Correctnesstestcase {
    ...
    public void Test1() {
        ...
        siddhiManager.setExtension("singlenode:agg", DimensionAggregator.class);
        ...
        String inStreamDefinition = "@config(async = 'true')\n" + //@config(async = 'true')@plan:async
                ...
                "value double, centers int);";

        String query = ("" +
                "\n" +
                "from inStream " +
                "select machine, time, dimension, str:concat(machine, '-', dimension) as partitionId, uTime, value, centers " +
                ...
                "select machine, time, dimension, singlenode:agg(value, centers, 10) as probaility " +
            ...
            for (; results.hasNext(); ) {
                ...
                if (DebsMetaData.getMetaData().keySet().contains(dimension) && !value.toString().contains("#string")) {
                    arr.add(new Object[]{
                            ...
                            time.getLocalName(),
                            dimension,
                            UnixConverter.getUnixTime(timestamp.getString()),
                            value.getDouble(),
